            ...
            <ion-list>
                <ion-card @click="openModal(order)" v-for="order in orders" :key="order.id" class="loii-align-center">
                    <ion-card-header>
                        ...
                        <ion-card-subtitle>
                            <p>To be Delivered To: {{order.user_id ? order.user.name : "Not Assigned"}}</p>
...
import Modal from '/src/layouts/DeliveryOrder.vue';
...
export default {
    ...
    methods : {
        initialize(){
            this.$axios.get('/delivery_man/v1/order/all').then(({data}) => {
                this.orders = data
            })
        },
        getStatus(status){
